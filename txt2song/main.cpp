...
void Ugh (const char *msg)  {Ugh (CC(msg));}
...
{ char *p, shrp;
  ...
  ubyte len, step, f, chd [4], nChd, i, rollOfs = 0;
   while (l && (*b == ' '))  {b++; l--;}
   if (l == 0)                         Ugh ("Missin duration");
   for (len = 0;  len < l;  len++)  if (b [len] == ' ') break;
   if (! DoDur (b, len))               Ugh ("Bad duration");
   ...
   // velocity change?
      if ((l >= 2) && (*b == 'V')) {
         step = b [1];
         if ((step < '0') || (step > '9'))  Ugh ("Bad velocity");
   ...
   // regular octNoteNote...
      if (! got) {
         ...
         if (l == 0)                        Ugh ("Missin Nt after Oct");
         if (! (p = StrCh (NoteSym, *b)))   Ugh ("Bad note");
         step = p - NoteSym;                                 b++;  l--;
      ...
      // ok, got dur and step - create the note
         if (NE+2 >= MAX_EVT)          Ugh ("Hit max events");
         if (Grace) {
            if (Time < (M_WHOLE/32))   Ugh ("Gracenote can't start bar 1");
         ...
         if (l && (*b == '+')) {       // CHORD !
            ...
            if ((NE+2*nChd) >= MAX_EVT)  Ugh ("Hit max events +");
            for (i = 0;  i < nChd;  i++) {
               step += chd [i];
...
// special-ish ones...
   ...
   if (! StrCm (b, "marker"))
      Ugh ("use section.  not marker.  sorry.");
   if (! StrCm (b, "section")) {
      if (NMrk >= BITS (Mrk))  Ugh ("tooo many sections");
      if (NMrk && (Time <= Mrk [NMrk-1].t))
         Ugh ("section's time is BEFORE previously found section - "
...
NMrk, Mrk[NMrk].s, Mrk[NMrk].t, TmS (db,Mrk[NMrk].t));
   ...
   if (! StrCm (b, "cue")) {
      if (NCue >= BITS (Cue))  Ugh ("tooo many cue lines");
      if (NCue && (Time <= Cue [NCue-1].t))
         Ugh ("this cue time is BEFORE a prev cue - "
...
// get control id;  add it to list if new
   ...
   if (c >= NCtrl) {                   // new dude
      if (StrLn (b) > (sizeof (WStr)-1))
                                 Ugh ("control name too long");
      if (NCtrl >= BITS (Ctrl))  Ugh ("too many controls");
      StrCp (Ctrl [NCtrl++], b);
   }

// ok, parse the wierd ones first (tsig, ksig)
   if      (! StrCm (b, "TSig")) {
      if (! (p2 = StrCh (p, '/')))     Ugh ("TSig Missing /");
      p3 = StrCh (++p2, '/');
      n = (ubyt2) Str2Int (p);
      d = (ubyt2) Str2Int (p2);
      for (i = 0;  i < 8;  i++) if ((1 << i) == d) break;
      if (i >= 8)                      Ugh ("bad TSig denom");
      v = (i << 8) | n;
      if (p3) {                  // got subbeat?
         i = (ubyte) Str2Int (p3+1);
         if ((i >= 1) && (i <= 8))  v |= ((i-1) << 12);
         else                          Ugh ("bad TSig subbeat");
      }
   ...
   if (NE >= MAX_EVT)  Ugh ("tooo many events");
   ...
   for (;;) {
      ...
      if (StrCm (chd, "|")) {          // | really does nothin, just legibility
         if (StrCm (chd, "/")) {       // / only bumps Time
            if (NCue >= BITS (Cue))  Ugh ("tooo many chords");
            Cue [NCue].t     = Time;
...
{ File  f;
  ...
  ubyt4     pLn;
   ...
   if ((f.DoText (ErrFN, nullptr, DoLine))) {
      ...
      StrCp (ErrFN, fn);   ErrLine = 0;
      if ((msg = f.DoText (ErrFN, nullptr, DoLine)))  Ugh (msg);
   }
...
//DBG("FN=`s line=`d NE=`d Time=`d", ErrFN, ErrLine, NE, Time);
   ...
   if (unr) {
      if (NSct >= BITS (Sct))
                      Ugh ("too many sections in unroll");
      for (i = 0;  i < NMrk;  i++)  if (! StrCm (Mrk [i].s, b))  break;
      if (i >= NMrk)  Ugh ("unknown section in unroll");
      StrCp (Sct [NSct++], b);
      return nullptr;
   }
...
// default scale/artic?
   if (*b == '$')  {
      if (l < 8)             Ugh ("Bad scale");
      if (l > 8) {
         if (! StrCh (ArtcSym, b [8]))
                             Ugh ("Bad default Artic");
         Artc = b [8];
      }
...
// read top .txt file line by line n do each track
   if ((msg = f.DoText (FN, nullptr, DoLine)))  Ugh (msg);
