...
// recreate dest dir
   StrFmt (dFnd, "`s/song/found", App.Path (s, 'd'));
   d.Kill (dFnd);   d.Make (dFnd);

// ok copy it to song/found
...
TRC("SongKill `s", FL.lst [FL.pos]);
   StrCp (dr, FL.lst [p]);   App.Path (t, 'd');   StrAp (t, "/song/");
   if (MemCm (dr, t, StrLn (t)))
      {Gui.Hey ("songKill only works in song dir");   return;}
